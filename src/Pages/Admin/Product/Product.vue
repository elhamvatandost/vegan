<template>
  <v-form>
    <v-expansion-panels v-model="panel" multiple>
      <v-expansion-panel>
        <v-expansion-panel-header>اضافه کردن محصول</v-expansion-panel-header>
            ...
            <v-row>
              ...
              <v-col cols="12" sm="6">
                <autocomplete
                  ...
                  @selected="selected"
                  @clear="cleared"
                  @input="$v.groupId.$touch()"
                  @blur="$v.groupId.$touch()"
                  :request-headers="authHeaders"
                ></autocomplete>
                <span style="color:red">{{catErr.toString()}}</span>
              </v-col>‍
              <v-btn color="success" class="mr-2" @click="addProduct" :disabled="$v.$invalid">
                ‍ ثبت اطلاعات
                ‍
              </v-btn>
            </v-row>
      ...
      <v-expansion-panel>
        <v-expansion-panel-header>لیست محصولات</v-expansion-panel-header>
        <v-expansion-panel-content>
          <v-container>
            <v-row>
              <v-col cols="12">
...
export default {
  ...
  computed: {
    ...
    catErr() {
      const errors = [];
      if (!this.$v.groupId.$dirty) return errors;
      !this.$v.groupId.required && errors.push("گروه محصولات را وارد کنید");
      return errors;
    },
    tagErr() {
      const errors = [];
  ...
  methods: {
    ...
    del(item) {
      if (confirm("آیا از حذف اطمینان دارید؟")) {
        this.$store.dispatch("rmvProduct", item._id).then(() => {
          this.text = "محصول شما حذف شد.";
          this.snackbar = true;
        });
      }
    },
    addProduct() {
      this.$store
        ...
        .then(() => {
          ...
          this.groupId = "";
          this.tags = [];
          this.text = "محصول شما ثبت شد .";
          this.snackbar = true;
        });
    },
    selected(a) {
      this.groupId = a.value;
    },
    cleared() {
      this.groupId = "";
    }
