    ...
    <v-data-table :headers="headers" :items="getList" sort-by="_id" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>گروه محصولات</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-dialog v-model="dialog" max-width="1500px">
            <template v-slot:activator="{ on }">
              <v-btn color="light-green darken-3" dark class="mb-2" v-on="on">گروه محصولات جدید</v-btn>
            </template>
            <v-card>
              <v-card-title>
  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      if (confirm("آیا از حذف اطمینان دارید ؟")) {
        this.$store.dispatch("rmvGroup", index).then(() => {
          this.text = " گروه محصولات حذف شد .";
          this.snackbar = true;
        });
      }
    },
    ...
    save() {
      if (this.editedIndex != -1) {
        this.$store
          ...
          .then(() => {
            this.text = "گروه محصولات مورد نظر ویرایش شد .";
            this.snackbar = true;
          });
      } else {
        this.$store.dispatch("addGroup", this.editedItem).then(() => {
          this.text = "گروه محصولات اضافه شد .";
          this.snackbar = true;
        });
      }
      this.close();
